...
Put something like the following in your config.yaml to configure: as per kodiupdate plugin
    ...
    audiodb:
        key: secretkey
"""
#Requires mb_albumid & mb_albumartistid from same album
...
def artist_info():
    ...
    artistid = 'mb_albumartistid:' + mb_albumartistid
...
def album_info():
    ...
    <label>{11}</label>""".format(data[3], data[1], data[0], data[4], data2["strStyle"] or '', data2["strMood"] or '',
        data2["strTheme"] or '', comp, data2["strDescriptionEN"] or '', data[7], rel_date, data[6])
        print (ainfo, file=open(albumnfo,"w+"))
    paths('album','path')
    paths('album','thumb')
    print ("    <thumb>{0}</thumb>".format(data2["strAlbumThumb"] or ''), file=open(albumnfo,"a"))
    print ("    <rating>{0}</rating>".format(int(data2["intScore"] or '')/2), file=open(albumnfo,"a"))
...
artistnfo = os.path.join(info[1][0], 'artist.nfo')
